...
function populateCrawler(crawlerDetail){
    ...
    var valid_rules = crawlerDetail.valid_rules;
    populateFieldMappingTable($.parseJSON(field_mapping));
    var valid_rules_objs = $.parseJSON(valid_rules)
    for(var obj in valid_rules_objs){
        valid_rules_objs[obj].uuid = Math.uuid()
    }
    populateValidateRulesTable(valid_rules_objs);
